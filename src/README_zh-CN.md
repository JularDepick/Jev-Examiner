...
│   ├── app.js              # 服务入口: HTTP/HTTPS 服务、静态托管与错误兜底
...
| 模块 | 职责 |
|:---:|:---:|
| `api/app.js` | 创建 HTTP 或 HTTPS 服务,按强制加密开关拦截明文请求,转发请求至路由,可选托管前端静态资源,统一兜底错误响应 |
| `api/include/constants.js` | 集中存放端口、上游地址、模型标识、密钥来源策略、TLS 证书路径、阈值与长度上限等可调参数 |
...
| `api/include/httpUtils.js` | 读写 JSON 请求体、发送响应、跨域头、密钥来源解析与加密通道判定 |
| `pages/app.js` | 前端状态管理、问题集编辑、端点切换、直连官方 API 与结果渲染 |
...
- 密钥来源按调用粒度选择,支持调用方携带、服务端常量与自动回退三种策略
- 前端访问本地服务固定使用 HTTPS,服务端可启用 HTTPS 监听并强制拒绝明文请求,避免密钥在传输中暴露
