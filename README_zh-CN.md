...
| 密钥来源可选 | 每次调用可指定密钥来自调用方还是服务端常量,支持调用方优先的自动回退 |
| 传输加密 | 支持以 HTTPS 监听,可强制拒绝非加密请求,避免密钥在明文通道中暴露 |
...
API 密钥随请求传输,明文通道存在泄漏风险。同时提供证书与私钥路径即可启用 HTTPS 监听:

```
HTTPS_KEY_PATH=./key.pem HTTPS_CERT_PATH=./cert.pem npm start
```

在此基础上设置 `REQUIRE_HTTPS=true`,服务会拒绝一切非加密请求,回环来源同样不放行。若由反向代理终止 TLS,服务依据 `X-Forwarded-Proto` 判断请求是否加密,此时应确保服务不直接暴露。

### 选择密钥来源

调用方可在每次请求中指定密钥来源,取值 `auto` `client` `server`,分别表示调用方优先并回退服务端、仅用调用方、仅用服务端。未指定时取服务端默认策略(环境变量 `KEY_SOURCE`)。
...
前端默认直连 Jev 官方端点。官方端点仅放行白名单来源,浏览器以本地方式打开页面时会因跨域策略被拦截,此时可将 API 基地址改为本地服务 `https://localhost:8080/audit` 经其转发。本地服务固定使用 HTTPS,首次使用需先在浏览器中打开该地址并信任其自签名证书。
